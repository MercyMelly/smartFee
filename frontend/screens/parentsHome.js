...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Alert, ActivityIndicator, FlatList, Platform, KeyboardAvoidingView, ScrollView } from 'react-native';
...
export default function ParentDashboard() {
    ...
    const handleGenerateReceipt = useCallback(async (paymentId, transactionReference) => {
        ...
        try {
            ...
            const url = `${BASE_URL}/parents/payments/generate-receipt/${paymentId}`; // ✅ fixed path
    ...
    const navigateToPayFees = () => {
        ...
        navigation.navigate('payFees', params); 
    };
return (
                                ...
                                <View style={styles.profileCard}>
                                    ...
                                    {currentSelectedStudent.currentFeeDeadline && (
                                        <Text style={styles.feeNotes}>
                                            <Ionicons name="information-circle-outline" size={14} color="#757575" /> Next Deadline: {new Date(currentSelectedStudent.currentFeeDeadline.deadlineDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} ({currentSelectedStudent.currentFeeDeadline.term} {currentSelectedStudent.currentFeeDeadline.academicYear})
                                        </Text>
                                    )}
