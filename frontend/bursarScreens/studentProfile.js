...
const StudentProfile = () => {
  ...
  const fetchStudentProfile = async () => {
    try {
      const studentRes = await fetch(`http://10.71.113.17:3000/api/students/${admissionNumber}`);
      const studentData = await studentRes.json();
      setStudent(studentData);

      const paymentRes = await fetch(`http://10.71.113.17:3000/payments/student/${admissionNumber}`);
